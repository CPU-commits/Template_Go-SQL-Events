...
func testAccessesInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(accessPrimaryKeyColumns, accessColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
var (
	accessDBTypes = map[string]string{`ID`: `bigint`, `Token`: `text`, `ExpiresAt`: `date`, `IsRevoked`: `boolean`, `IDSession`: `bigint`, `CreatedAt`: `timestamp without time zone`}
	_             = bytes.MinRead
)
