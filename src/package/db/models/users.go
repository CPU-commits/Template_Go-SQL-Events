...
type userR struct {
	...
	IDUserAuths         AuthSlice      `boil:"IDUserAuths" json:"IDUserAuths" toml:"IDUserAuths" yaml:"IDUserAuths"`
...
func (o *User) GetIDUserAuths() AuthSlice {
	if o == nil {
		return nil
	}

	return o.R.GetIDUserAuths()
}

func (r *userR) GetIDUserAuths() AuthSlice {
	if r == nil {
		return nil
	}

	return r.IDUserAuths
}
...
// IDUserAuths retrieves all the auth's Auths with an executor via id_user column.
func (o *User) IDUserAuths(mods ...qm.QueryMod) authQuery {
...
// LoadIDUserAuths allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadIDUserAuths(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load auths")
	}

	var resultSlice []*Auth
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice auths")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on auths")
	}
	if err = results.Err(); err != nil {
	...
	if singular {
		object.R.IDUserAuths = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &authR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.IDUser {
				local.R.IDUserAuths = append(local.R.IDUserAuths, foreign)
				if foreign.R == nil {
					foreign.R = &authR{}
				}
				foreign.R.IDUserUser = local
...
// AddIDUserAuths adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.IDUserAuths.
// Sets related.R.IDUserUser appropriately.
func (o *User) AddIDUserAuths(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Auth) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.IDUserAuths = append(o.R.IDUserAuths, related...)
	}
