...
func testUserToManyIDUserAuths(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.IDUserAuths().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadIDUserAuths(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDUserAuths); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.IDUserAuths = nil
	if err = a.L.LoadIDUserAuths(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.IDUserAuths); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpIDUserAuths(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddIDUserAuths(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.IDUserAuths[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.IDUserAuths[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.IDUserAuths().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	userDBTypes = map[string]string{`ID`: `bigint`, `Email`: `text`, `Name`: `text`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
